...
from src.helpers.helpers import clean_up_age_data, clean_up_gender_data, only_ten_sec
...
def test_only_ten_sec():
    ...
    result_age, result_gender, result_filename, result_labels = only_ten_sec(
        ecg_len, age, gender, filename, labels
    )
    assert np.array_equal(age[[0, 3]], result_age)
    assert np.array_equal(gender[[0, 3]], result_gender)
    assert np.array_equal(filename[[0, 3]], result_filename)
    assert np.array_equal(labels[[0, 3]], result_labels)
